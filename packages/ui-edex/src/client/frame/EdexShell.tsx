...
/** The NovaFleet title bar strip atop the center workspace card (~18px). The
 *  reshaped layout frame is inset below it so the workspace content is never
 *  covered by the chrome. */
export const CENTER_TITLE_BAR_HEIGHT = '18px'
...
export function EdexShell({
  ...
  useLayoutEffect(() => {
    ...
    // The empty top panel takes no space (TOP_BAR_HEIGHT = 0); the workspace
    // frame is inset below the center title bar strip so the NovaFleet
    // chrome never covers the original UI.
    frame.style.top = CENTER_TITLE_BAR_HEIGHT
  ...
  return (
    ...
    >
      ...
      {/* The ORIGINAL UI wrapped in the standard widget chrome: a NovaFleet
          title bar strip over the center workspace card (the same 1px border
          + card-surface chrome as the side widgets). The inner WidgetSection
          stays transparent so the original UI shows through. */}
      <section className={css.centerWidget}>
        <div className={css.centerTitle}>NOVAFLEET WORKSPACE</div>
