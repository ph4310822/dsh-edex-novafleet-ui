...
/** The default theme color — the NovaFleet mission-control blue. */
...
export const THEME_COLOR_PRESETS: readonly ThemeColorPreset[] = Object.freeze([
  { id: 'analyzed', labelKey: 'edex.preset.analyzed', color: '#3b82f6' },
  { id: 'amber', labelKey: 'edex.preset.amber', color: '#fbbf24' },
  { id: 'cyan', labelKey: 'edex.preset.cyan', color: '#60a5fa' },
  { id: 'violet', labelKey: 'edex.preset.violet', color: '#c792ea' },
  { id: 'blue', labelKey: 'edex.preset.blue', color: '#7ec8e3' },
])

/** The semantic accents that stay fixed across theme colors (NovaFleet reference: green success / yellow warn / red error / light-blue info). */
...
/**
 * Derive the full NovaFleet family from one accent color. The primary follows
 * the accent; the surface family (dim slate, muted gray-blue border, blue-gray
 * card panel) is fixed from the NovaFleet reference analysis so the shell
 * canvas reads as the reference's dashboard surface rather than a derived
 * tint of the accent.
 */
...
export function tokenOverridesFor(palette: EdexPalette): ThemeTokenOverrides {
  return {
    // Backgrounds share the shell panel color (the blue-gray card surface),
    // so the original UI in the center reads as the same surface as the
    // themed panels — not a separate pure-black canvas.
    '--dsw-alias-bg-base': both('#1e2938'),
    '--dsw-alias-bg-layer-1': both('#1e2938'),
    '--dsw-alias-bg-layer-2': both('#1e2938'),
    '--dsw-alias-bg-overlay': both('#1e2938'),
    ...
    '--dsw-alias-state-success-primary': both('#22c55e'),
    ...
    '--dsw-specific-sidebar-fill': both('#1e2938'),
    '--dsw-specific-input-major': both('#1e2938'),
  }
}
